...
tracking_confidence = 0.3  # minimum confidence to consider a detection
person_confidence = 0.3  # minimum confidence to consider a person detection
bike_confidence = 0.3  # minimum confidence to consider a bike detection
helmet_confidence = 0.3  # minimum confidence to consider a helmet detection
...
# ✅ Explicitly import and allow all classes used by YOLO
from ultralytics.nn.tasks import DetectionModel
import torch.nn as nn
from torch.nn.modules.container import Sequential

torch.serialization.add_safe_globals([DetectionModel, nn.Module, Sequential])

# ✅ Load your pretrained model (trusted source)
coco_yolo = YOLO('models/best_260424_0028.pt')
...
def track_on_video(video_path, tracking_assignment, conf, output_path=None):
    ...
    while cap.isOpened():
        ...
        if success:
            ...
            print(f"Detections this frame: {len(result_list)}")
...
if __name__ == '__main__':
    ...
    # ]
    tracking_assignment = [
    (oiv7_yolo, [322, 594, 381, 42]),  # person, man, woman and bicycle
    (my_yolo, [0, 1])  # without helmet, with helmet
]
